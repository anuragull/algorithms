...
def reverse_subarray(start, end):
  ...
  while cur != end:
    # set first one to 
    nex = cur.next
    # set the current next to previous one 
    cur.next = pre
    # set the cur 
    pre = cur
    # set the pre 
...
def reverse(head):
  ...
  # two pointers approach to find places of the break
  while fp != None:
    # start of the subgroup
    if fp.data % 2 == 0:
      sp = fp.next
      last_sp = sp
      while (sp and sp.data % 2 == 0):
        last_sp = sp
        sp = sp.next
      # reverse subarray
      end, start = reverse_subarray(fp, last_sp)
      start.next = sp
      if prev_fp:
        prev_fp.next = end
      else:
